...
#Include "enemy-scanner.ahk"

/**
 * 主流程模块
 * 负责自动升级的核心逻辑：检测颜色、点击按钮、扫描敌人区域
 */
 ...
 * @param enemyAreaConfig 敌人区域配置对象
 */
CheckButtons(windowTitle, targetColors, colorVariance, buttonCoords, selectButtonImage, selectImageVariance, enemyAreaConfig) {
    ; 尝试先点击"选择"按钮
    ...
    ScanEnemyArea(
        windowTitle,
        enemyAreaConfig.StartX,
        enemyAreaConfig.StartY,
        enemyAreaConfig.EndX,
        enemyAreaConfig.EndY,
        enemyAreaConfig.LineSpacing,
        enemyAreaConfig.ScanStep,
        enemyAreaConfig.MouseSpeed
    )
}

/**
 * 初始化调试标记
 * 在启动时绘制按钮坐标和扫描区域顶点的红色标记
 * @param windowTitle 窗口标题
 * @param buttonCoords 按钮坐标数组
 * @param enemyAreaConfig 敌人区域配置对象
 * @param enableDebug 是否启用调试标记
 * @param markerRadius 按钮标记半径
 * @param vertexMarkerRadius 顶点标记半径
 */
InitDebugMarkers(windowTitle, buttonCoords, enemyAreaConfig, enableDebug, markerRadius, vertexMarkerRadius) {
    if (!enableDebug) {
        return
    }
    ...
    ; 绘制扫描区域的两个顶点（左上角和右下角）
    EnemyAreaVertices := [[enemyAreaConfig.StartX, enemyAreaConfig.StartY], [enemyAreaConfig.EndX, enemyAreaConfig.EndY]]
    DrawCoordMarkers(windowTitle, EnemyAreaVertices, vertexMarkerRadius)
